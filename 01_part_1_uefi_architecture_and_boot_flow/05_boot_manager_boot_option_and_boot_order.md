# Boot Manager、Boot Option 與開機順序

> 狀態：Draft
> 文件用途：本檔為章節撰寫大綱，內容、規格版本與平台資料仍需由章節負責人補充及驗證。

## 1. 文件目的
- 說明「Boot Manager、Boot Option 與開機順序」在 UEFI 架構與開機流程 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
## 3. 建議內容大綱
### 3.1 Boot####、BootOrder 與 BootNext Variable
### 3.2 EFI_LOAD_OPTION 與 Device Path
### 3.3 Platform Boot Policy 與預設順序
### 3.4 Removable Media、Fallback 與恢復路徑
### 3.5 Hotkey、Front Page、Timeout 與 Setup
### 3.6 Boot Option 建立、刪除、去重與排序
### 3.7 Secure Boot、HTTP/PXE 與 OS Loader 的影響
