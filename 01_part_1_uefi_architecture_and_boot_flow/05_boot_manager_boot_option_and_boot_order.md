...
### 5.1.2 Boot####、BootOrder 與 BootNext
...
### 5.1.5 Driver Option 與 Boot Option 的差異
...
- UEFI Shell：`bcfg boot dump`、`map -r`、`devices`、`drivers`
- OS 工具：Linux `efibootmgr -v`
- EDK II Debug Log：Variable、BDS、Device Path、Image Loader 相關訊息
- 自製 Dump 工具：列出 Attributes、Description、Device Path Text 與 OptionalData 長度
...
### 5.7.5 OS Loader 建立或更新 Boot Option
