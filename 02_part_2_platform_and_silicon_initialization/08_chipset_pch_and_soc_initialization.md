# Chipset、PCH 與 SoC 初始化

> 狀態：Draft
> 文件用途：本檔為章節撰寫大綱，內容、規格版本與平台資料仍需由章節負責人補充及驗證。

## 1. 文件目的
- 說明「Chipset、PCH 與 SoC 初始化」在 平台與矽晶片初始化 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
### 3.1 PCH／SoC 初始狀態與初始化階段

### 3.2 Silicon Policy、Strap 與 Fuse 的資料來源

### 3.3 Host Bridge、DMI／Fabric 與內部匯流排設定

### 3.4 PCH 內建周邊的 Enable／Disable Policy

### 3.5 Interrupt、DMA、IOMMU 與資源配置

### 3.6 Lockdown、End-of-POST 及設定鎖定

### 3.7 Stepping 差異、Errata 與驗證方式
