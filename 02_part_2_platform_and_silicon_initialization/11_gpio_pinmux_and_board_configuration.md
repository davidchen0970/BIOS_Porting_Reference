...
## 1. 文件目的
- 說明「GPIO、Pinmux 與板級設定」在 平台與矽晶片初始化 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
### 3.1 GPIO Community、Group、Pad 與編號方式

### 3.2 Pad Mode、Direction、Pull、Drive、Reset Domain 與 Ownership

### 3.3 Native Function 與 GPIO Pin Multiplexing 對照表

### 3.4 GPIO Table 的產生、載入時機與 SKU Override

### 3.5 早期開機、PEI、DXE 與 Runtime 的讀寫限制

### 3.6 Interrupt、Wake、Debounce 與安全鎖定

### 3.7 Register Dump、Loopback 與板級量測驗證
