# PCD、Build Flag 與平台組態

> 狀態：Draft
> 文件用途：本檔為章節撰寫大綱，內容、規格版本與平台資料仍需由章節負責人補充及驗證。

## 1. 文件目的
- 說明「PCD、Build Flag 與平台組態」在 EDK II、建置系統與設定資料 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
### 3.1 PCD Token Space、型別與資料生命週期

### 3.2 FixedAtBuild、Patchable、FeatureFlag、Dynamic 與 DynamicEx

### 3.3 DSC、DEC、INF、FDF 間的設定與覆寫優先序

### 3.4 Build Target、Architecture、Tool Chain 與巨集

### 3.5 SKU ID、Default Store 與平台組態選擇

### 3.6 PCD Database 產生、存取與除錯

### 3.7 設定未生效、型別不符與跨模組相依排查
