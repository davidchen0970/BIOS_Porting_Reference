...
## 1. 文件目的
- 說明「工具鏈、建置流程與映像檔產出」在 EDK II、建置系統與設定資料 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
### 3.1 BaseTools、Compiler、Assembler、Linker 與版本需求

### 3.2 edksetup、Conf 目錄及建置環境初始化

### 3.3 build 命令、Target、Toolchain、Architecture 與 DSC

### 3.4 AutoGen、編譯、連結、GenFds 與映像檔封裝流程

### 3.5 Build／FV／Ffs／Map／Report 產物判讀

### 3.6 警告政策、重現建置與 CI 整合

### 3.7 建置失敗、映像超限與工具鏈差異排查
