# USB Host Controller 與開機裝置

> 狀態：Draft
> 文件用途：本檔為章節撰寫大綱，內容、規格版本與平台資料仍需由章節負責人補充及驗證。

## 1. 文件目的
- 說明「USB Host Controller 與開機裝置」在 周邊裝置與匯流排初始化 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
### 3.1 xHCI／EHCI 控制器 PCI 枚舉與 MMIO 賦能

### 3.2 USB Port Routing、Port Disable、OC Pin 與板級對應

### 3.3 Host Controller Reset、Legacy Support 與 Ownership

### 3.4 Hub、Mass Storage、Keyboard 及裝置枚舉

### 3.5 Hot-plug、連線速度、供電與裝置偵測策略

### 3.6 USB Device Path、Block I/O 與開機選項建立

### 3.7 USB Debug Port、Protocol Trace 與開機失敗排查
