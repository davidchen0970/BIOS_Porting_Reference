# SATA、NVMe、eMMC 與儲存裝置

> 狀態：Draft
> 文件用途：本檔為章節撰寫大綱，內容、規格版本與平台資料仍需由章節負責人補充及驗證。

## 1. 文件目的
- 說明「SATA、NVMe、eMMC 與儲存裝置」在 周邊裝置與匯流排初始化 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
### 3.1 SATA／AHCI、NVMe 與 eMMC 控制器初始化

### 3.2 PCI 枚舉、BAR／MMIO、Bus Master 與控制器賦能

### 3.3 Port Enable、Hot-plug、Lane Mapping 與裝置偵測

### 3.4 Identify／Namespace／LUN 與容量資訊取得

### 3.5 Block I/O、Disk I/O、Partition 與檔案系統 Protocol

### 3.6 Device Path、Boot Option 及 RAID／VMD 相依性

### 3.7 逾時、鏈路錯誤、媒體缺失與開機降級策略
