# Graphics、GOP 與主控台初始化

> 狀態：Draft
> 文件用途：本檔為章節撰寫大綱，內容、規格版本與平台資料仍需由章節負責人補充及驗證。

## 1. 文件目的
- 說明「Graphics、GOP 與主控台初始化」在 周邊裝置與匯流排初始化 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
### 3.1 顯示控制器偵測、Primary Display Policy 與資源配置

### 3.2 VBIOS／UEFI Graphics Driver 載入與相容性

### 3.3 GOP Mode、Framebuffer、EDID 與解析度選擇

### 3.4 ConOut／ConIn／ErrOut 與 Console Variable

### 3.5 Logo、Quiet Boot、Text／Graphics Mode 切換

### 3.6 多顯示器、Headless 與 Serial Console Policy

### 3.7 黑畫面、Mode Set 失敗與畫面輸出驗證
