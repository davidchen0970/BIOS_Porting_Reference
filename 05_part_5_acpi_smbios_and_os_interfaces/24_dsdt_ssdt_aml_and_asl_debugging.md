# DSDT、SSDT、AML 與 ASL 除錯

> 狀態：Draft
> 文件用途：本檔為章節撰寫大綱，內容、規格版本與平台資料仍需由章節負責人補充及驗證。

## 1. 文件目的
- 說明「DSDT、SSDT、AML 與 ASL 除錯」在 ACPI、SMBIOS 與作業系統介面 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
## 3. 建議內容大綱
### 3.1 ASL、AML、Namespace 與 Object Model
### 3.2 Device、Method、Name、Scope 與 OperationRegion
### 3.3 _STA、_CRS、_PRT、_DSM 等常用物件
### 3.4 DSDT 與 SSDT 的拆分原則
### 3.5 iasl 編譯、反編譯與 Warning 管理
### 3.6 OS ACPI Debug、Method Trace 與 Namespace 檢查
### 3.7 硬體存取同步、Mutex 與副作用控制
