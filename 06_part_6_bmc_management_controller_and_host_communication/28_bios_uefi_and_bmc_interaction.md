...
## 1. 文件目的
- 說明「BIOS／UEFI 與 BMC 互動」在 BMC、管理控制器與主機通訊 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
## 3. 建議內容大綱
### 3.1 開機階段 BIOS 與 BMC 的責任分工
### 3.2 Power State、Reset Cause 與 Boot Progress
### 3.3 FRU、Inventory、Sensor 與平台資訊交換
### 3.4 Watchdog、POST Code 與 SEL Event
### 3.5 BIOS 設定同步及帶外設定
### 3.6 Firmware Version、Update Coordination 與 Recovery
### 3.7 介面不可用、逾時與降級行為
