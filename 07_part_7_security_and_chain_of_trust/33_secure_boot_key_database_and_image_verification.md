# Secure Boot、Key Database 與映像檔驗證

> 狀態：Draft
> 文件用途：本檔為章節撰寫大綱，內容、規格版本與平台資料仍需由章節負責人補充及驗證。

## 1. 文件目的
- 說明「Secure Boot、Key Database 與映像檔驗證」在 安全性與信任鏈 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
## 3. 建議內容大綱
### 3.1 Secure Boot 信任模型與驗證時機
### 3.2 PK、KEK、db、dbx 的用途與權限
### 3.3 Setup Mode、User Mode 與 Custom Mode
### 3.4 PE/COFF Authenticode 與映像檔驗證
### 3.5 金鑰建立、簽署、輪替及撤銷
### 3.6 Violation、Audit Mode 與 Recovery
### 3.7 測試案例、負向測試與出廠 Provisioning
