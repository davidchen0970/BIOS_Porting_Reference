# SMM、MM、Variable 與 Flash Protection

> 狀態：Draft
> 文件用途：本檔為章節撰寫大綱，內容、規格版本與平台資料仍需由章節負責人補充及驗證。

## 1. 文件目的
- 說明「SMM、MM、Variable 與 Flash Protection」在 安全性與信任鏈 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
### 3.1 SMM／MM Entry、Dispatcher 與通訊介面

### 3.2 SMRAM／MMRAM 配置、鎖定與存取控制

### 3.3 SMM Variable、SPI Write 與更新服務

### 3.4 SMM Communication Buffer 驗證與 TOCTOU 防護

### 3.5 BIOS Lock、SMM_BWP、PR Range 與 Flash Protection

### 3.6 S3 Resume、Runtime 及錯誤路徑的保護一致性

### 3.7 滲透測試、負向測試與安全事件紀錄
