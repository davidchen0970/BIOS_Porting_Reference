# Dual Image、Fail-safe 與 Recovery Flow

> 狀態：Draft
> 文件用途：本檔為章節撰寫大綱，內容、規格版本與平台資料仍需由章節負責人補充及驗證。

## 1. 文件目的
- 說明「Dual Image、Fail-safe 與 Recovery Flow」在 韌體更新、復原與維護 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
### 3.1 Active／Backup／Recovery Image 佈局與選擇邏輯

### 3.2 Boot Count、Health Check 與成功開機判定

### 3.3 更新先後順序、映像同步與原子切換

### 3.4 Watchdog、驗證失敗與自動回退條件

### 3.5 Recovery Strap、按鍵、BMC 與外部 Programmer 路徑

### 3.6 兩份映像皆失效時的最小復原能力

### 3.7 斷電注入、反覆失敗及版本交錯測試
