# Serial Log、POST Code、Trace 與 Debug Toolkit

> 狀態：Draft
> 文件用途：本檔為章節撰寫大綱，內容、規格版本與平台資料仍需由章節負責人補充及驗證。

## 1. 文件目的
- 說明「Serial Log、POST Code、Trace 與 Debug Toolkit」在 Debug、效能與測試 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
### 3.1 Serial Port、Baud Rate、Debug Level 與早期輸出

### 3.2 POST Code 埠、編碼表與階段對照

### 3.3 EDK II DEBUG、ASSERT、Performance 與 Trace

### 3.4 JTAG、ICE、Logic Analyzer、Oscilloscope 適用時機

### 3.5 UEFI Shell、Chipsec、RW／Register 工具與 Dump

### 3.6 Log 時間戳、版本、重現條件與收集封裝
