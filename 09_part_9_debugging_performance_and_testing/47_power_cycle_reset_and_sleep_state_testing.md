# S3、S4、S5、Warm Reset 與 Cold Boot 測試

> 狀態：Draft
> 文件用途：本檔為章節撰寫大綱，內容、規格版本與平台資料仍需由章節負責人補充及驗證。

## 1. 文件目的
- 說明「S3、S4、S5、Warm Reset 與 Cold Boot 測試」在 Debug、效能與測試 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
### 3.1 Cold Boot、AC Cycle、DC Cycle 與 G3→S5→S0

### 3.2 Warm Reset、Cold Reset、Global Reset 與 Watchdog Reset

### 3.3 S3／S4／S5 進入、喚醒與資料保留

### 3.4 Reset Cause、Wake Source 與事件紀錄驗證

### 3.5 不同電源間隔、循環次數與環境條件

### 3.6 周邊、網路、儲存與 BMC 協同狀態

### 3.7 失敗率、首失迭代、Log 與自動化控制
