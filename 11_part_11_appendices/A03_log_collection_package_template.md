# Log 收集 Package Template

> 狀態：Draft
> 文件用途：本檔為章節撰寫大綱，內容、規格版本與平台資料仍需由章節負責人補充及驗證。

## 1. 文件目的
- 說明「Log 收集 Package Template」在 附錄 中的角色。
- 定義本章涵蓋範圍、非涵蓋範圍與預期讀者。
...
### 3.2 必收項目：Serial Log、POST Code、Register Dump、ACPI／SMBIOS Table

### 3.3 韌體、硬體、工具、OS 與重現條件 Metadata

### 3.4 選收項目：Bus Trace、Waveform、Screenshot、Crash Dump

### 3.5 時間戳、時區、Log 對時與循環編號

### 3.6 敏感資料遮蔽、Hash 與封裝完整性
